...
mimic = Path('src/main/java/com/provismet/cursedspawners/entity/SpawnerMimicEntity.java')
text = mimic.read_text()
old = '        this.entityData.define(DISPLAY_ENTITY, entityTagFromSpawnData(this.spawnData));'
new = '        // Entity calls this during its superclass constructor, before this class\'s field initializers run.\n        // Seed the tracked display with a standalone default tag; loadSpawnerData/readAdditionalSaveData\n        // will replace it after construction.\n        this.entityData.define(DISPLAY_ENTITY, defaultSpawnData().getCompound("entity").copy());'
assert text.count(old) == 1, 'SpawnerMimicEntity defineSynchedData target changed'
text = text.replace(old, new)
mimic.write_text(text)

particle = Path('src/main/java/com/provismet/cursedspawners/client/particle/AOEChargingParticle.java')
particle.write_text(r'''/* MODIFIED unofficial Forge 1.20.1 backport; see NOTICE and LICENSE. */
...
old_version = 'mod_version=1.0.0+1.20.1-forge-backport'
new_version = 'mod_version=1.0.1+1.20.1-forge-backport'
assert props_text.count(old_version) == 1, 'gradle.properties version target changed'
props.write_text(props_text.replace(old_version, new_version))
